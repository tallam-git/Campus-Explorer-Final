...
open class ZoomAnimatedTransitioningDelegate: NSObject, UIViewControllerTransitioningDelegate {
    ...
    func initialize() {
        ...
        // Append it to the appropriate window for the current scene
    ...
    private func animationParams(using transitionContext: UIViewControllerContextTransitioning) -> (TimeInterval, () -> Void, (Any) -> Void) {
        ...
        if let referenceImageView = referenceImageView {
            ...
            if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
                ...
                statusBarManager.isStatusBarHidden && !toViewController.prefersStatusBarHidden && referenceSlideshowViewFrame.origin.y != parent.referenceSlideshowViewFrame?.origin.y {
        ...
        let completion = { (_: Any) in
            ...
            if completed {
                ...
                if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
                    let window = windowScene.windows.first {
                    window.removeGestureRecognizer(self.parent.gestureRecognizer)
                }
                // Unpauses slideshow
